...
**Full-stack local guide (no eSignet, matches this folder’s compose):** [docs/INJI_LOCAL_STACK.md](./docs/INJI_LOCAL_STACK.md)
...
## Local Postgres-backed issuance (no eSignet)

The default `docker-compose.yaml` in this folder is configured for **local Postgres person data**, **Certify-as-authorization-server** (pre-authorized code), and **no CSV/mock identity file**. See **[docs/LOCAL_POSTGRES_ISSUANCE.md](./docs/LOCAL_POSTGRES_ISSUANCE.md)** for prerequisites, ports, Postman flow, and how this differs from the Collab/eSignet + CSV farmer quickstart below.
...
- Database (PostgreSQL): `localhost:5434` by default, or `localhost:${INJI_STACK_POSTGRES_PORT}` when overridden
- Adminer: `localhost:9050` by default, or `localhost:${INJI_STACK_ADMINER_PORT}` when overridden
