...
public class PreAuthorizedCodeService {
    ...
    /**
     * When set (e.g. {@code personId}), the pre-authorized access token {@code sub} claim is set to this claim's
     * string value instead of the full JSON of all claims. Required for {@code PostgresDataProviderPlugin}, which
     * uses {@code sub} as the {@code :id} bind parameter for scope-query-mapping.
     */
    @Value("${mosip.certify.pre-auth.subject-claim-key:}")
    private String preAuthSubjectClaimKey;
    ...
    private String generateAccessToken(PreAuthCodeData codeData, String cNonce) {
        try {
            String identityData = resolvePreAuthIdentityData(codeData.getClaims());
    ...
    /**
     * Legacy behaviour: {@code sub} is the JSON-serialized claims map (used by mock / demo flows).
     * When {@link #preAuthSubjectClaimKey} is set and present in claims, {@code sub} is that claim's value only.
     */
    private String resolvePreAuthIdentityData(Map<String, Object> claims) throws JsonProcessingException {
        if (StringUtils.hasText(preAuthSubjectClaimKey) && claims != null && claims.containsKey(preAuthSubjectClaimKey)) {
            Object v = claims.get(preAuthSubjectClaimKey);
            if (v != null) {
                return String.valueOf(v);
            }
        }
        return objectMapper.writeValueAsString(claims != null ? claims : Collections.emptyMap());
    }
}