...
public class VCIssuanceUtil {
    ...
    private static final Logger log = LoggerFactory.getLogger(VCIssuanceUtil.class);
    ...
    private static boolean isValidLdpVCRequest(CredentialRequest credentialRequest, CredentialConfigurationSupportedDTO credentialConfigurationSupportedDTO) {
        List<String> requestContext = credentialRequest.getCredential_definition().getContext();
        List<String> supportedContext = credentialConfigurationSupportedDTO.getCredentialDefinition().getContext();
        List<String> requestTypes = credentialRequest.getCredential_definition().getType();
        List<String> supportedTypes = credentialConfigurationSupportedDTO.getCredentialDefinition().getType();

        if (requestContext.size() != supportedContext.size()) {
            log.warn("ldp_vc @context size mismatch for scope {}: request={} supported={}",
                    credentialConfigurationSupportedDTO.getScope(), requestContext, supportedContext);
            return false;
        }

        if (requestTypes.size() != supportedTypes.size()) {
            log.warn("ldp_vc type size mismatch for scope {}: request={} supported={}",
                    credentialConfigurationSupportedDTO.getScope(), requestTypes, supportedTypes);
            return false;
        }

        if (!new HashSet<>(supportedContext).containsAll(requestContext)) {
            log.warn("ldp_vc @context values mismatch for scope {}: request={} supported={}",
                    credentialConfigurationSupportedDTO.getScope(), requestContext, supportedContext);
            return false;
        }

        if (!new HashSet<>(supportedTypes).containsAll(requestTypes)) {
            log.warn("ldp_vc type values mismatch for scope {}: request={} supported={}",
                    credentialConfigurationSupportedDTO.getScope(), requestTypes, supportedTypes);
            return false;
        }
