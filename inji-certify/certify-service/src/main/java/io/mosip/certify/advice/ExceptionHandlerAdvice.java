...
public class ExceptionHandlerAdvice extends ResponseEntityExceptionHandler implements AccessDeniedHandler {
    ...
    private ResponseEntity<ResponseWrapper> handleInternalControllerException(Exception ex) {
        ...
        if(ex instanceof RecordConflictException) {
            RecordConflictException conflict = (RecordConflictException) ex;
            return new ResponseEntity<>(getResponseWrapper(conflict.getErrorCode(), ex.getMessage()), HttpStatus.CONFLICT);
        }
