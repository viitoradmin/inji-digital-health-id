...
The **Inji Verify API Test Rig** is designed for the execution of module-wise automation API tests for the inji verify module. This test rig utilizes **Java REST Assured** and **TestNG** frameworks to automate testing of the inji verify API functionalities. The key focus is to validate the VP Verification and related functionalities provided by the inji verify module.
...
This test rig covers only **external API endpoints** exposed by the inji verify module.
...
2. Open the property file from the folder `api-test\src\main\resources\config\injiVerify.properties` and update the property **injiVerifyBaseUrl** with URL respect to env:
...
2. Run the automation test suite JAR file:
   ```
   java -Dmodules=injiverify -Denv.user=api-internal.<env_name> -Denv.endpoint=<base_env> -Denv.testLevel=smokeAndRegression -jar apitest-injiverify-0.18.0-jar-with-dependencies.jar
   ```
...
## 4. **Run the Tests**
   ...
   - In the **Arguments** tab, add the necessary **VM arguments**:
     - **VM Arguments**:
       ```
       -Dmodules=injiverify -Denv.user=api-internal.<env_name> -Denv.endpoint=<base_env> -Denv.testLevel=smokeAndRegression```
...
- **jar**: Specify the name of the JAR file to execute. The version will change according to the development code version. For example, the current version may look like `apitest-injiverify-0.18.0-jar-with-dependencies.jar`.
...
2. Execute the tests as shown in the command above to validate inji verify API functionalities.
